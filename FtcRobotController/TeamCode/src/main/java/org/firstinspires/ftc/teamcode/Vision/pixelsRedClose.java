...
public class pixelsRedClose extends OpenCvPipeline {
    Mat HSVMat = new Mat();
    ...
    public Mat processFrame(Mat input) {
        ...
        //Config Scalars
        Scalar lowerBlue = new Scalar(pixelssRedClose.lowerBlueH,pixelssRedClose.lowerBlueS,pixelssRedClose.lowerBlueV);
        Scalar upperBlue = new Scalar(pixelssRedClose.upperBlueH,pixelssRedClose.upperBlueS,pixelssRedClose.upperBlueV);
        ...
        */
        ...
        for(MatOfPoint contour : contoursList){
            ...
            if(area>= pixelssRedClose.threshold && center.y>=pixelssRedClose.horizon){
                contThree.add(contour);
            }
        }

        //sort by X val
        contThree.sort(new sortArrRedClose());
...
//Comparator
class sortArrRedClose implements Comparator<MatOfPoint> {
    public int compare(MatOfPoint a, MatOfPoint b)
    {
        Moments aMom = Imgproc.moments(a);
        Moments bMom = Imgproc.moments(b);
        if(pixelsRedClose.calculateCentroid(aMom).x > pixelsRedClose.calculateCentroid(bMom).x){
            return 1;
        }else{
            return -1;
        }
