            ...
            switch (mRobotState) {
                ...
                case STACK:
                    ...
                    if(cycle==0||cycle==1) {
                        ...
                        points.add(new CurvePoint(STRAFE_BARRIER_RIGHT_2,0.62,0.62,10));
                    }
                    if(cycle == 1) {
                        if (robot.getPos().vec().distTo(points.get(points.size() - 1).toVec()) < 70) {
                            points.add(new CurvePoint(STACK, 0.3, 0.4, 10));
                        }
                    } else if (cycle == 2){
                        if (robot.getPos().vec().distTo(points.get(points.size() - 1).toVec()) < 70) {
                            points.add(new CurvePoint(STACK_2, 0.3, 0.4, 10));
                        }
                    } else {
                    ...
                    if(robot.getPos().vec().distTo(points.get(points.size() - 1).toVec()) < 2.5) {
                        ...
                        if (cycle == 1 || cycle == 2) {
                                    if (distanceL && distanceR) {
                                        robot.intake.intake_dropper.setPosition(0.43);
                                    ...
                                    if (distanceL && distanceR) {
                                        pos = pos2+.08;
                                        robot.intake.intake_dropper.setPosition(0.38);
                                    newState(State.BARRIER_CROSS);
                                    go = true;
                                    time.reset();
                                    secondTime.reset();
                ...
                case BARRIER_CROSS:
                    ...
                    if(cycle == 0) {
                        ...
                        if (secondTime.time() > .3 && secondTime.time() < .5) {
                            robot.v4b.grab();
                        } else if (secondTime.time() > .5 && secondTime.time() < .7) {
                            robot.v4b.open();
                        } else if (secondTime.time() > .7) {
                            robot.v4b.grab();
                        }
                ...
                case DEPOSIT:
                    ...
                    */
                        ...
                        if (pixelCase == 0) {
                            ...
                            } else if (secondTime.time() > 1.4 && secondTime.time() < 1.6) {
                                robot.v4b.vertical();
                            } else if (secondTime.time() > 1.6) {
                                robot.v4b.armIn();
